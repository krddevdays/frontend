            ...
            {props.events.map((event, index) => (
                <li key={index}
                    ...
                    <Link
                        ...
                        <p className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full ${new Date(event.start_date).getTime() >= Date.now() ? 'bg-green-100 text-green-800' : 'text-gray-500 bg-gray-100'}`}>
                            <EventDate startAt={new Date(event.start_date)}
